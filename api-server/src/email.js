...
async function sendVerificationEmail(email, nickname, token) {
  if (!transporter) {
    console.log('[EMAIL] SMTP not configured. Verification link:');
    console.log(`${SITE_URL}/api/auth/verify-email?token=${token}`);
    return;
  }
      ...
      html: `
        <div style="max-width:480px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
          ...
          <div style="padding:2rem;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;">
            ...
            <p style="font-size:0.9rem;color:#666;line-height:1.6;">아래 버튼을 클릭하여 이메일 인증을 완료해주세요.</p>
            ...
            <p style="font-size:0.75rem;color:#aaa;line-height:1.5;">이 링크는 24시간 동안 유효합니다.</p>
          </div>
        </div>
      `,
    });
...
async function sendPasswordResetEmail(email, nickname, token) {
  if (!transporter) {
    console.log('[EMAIL] SMTP not configured. Reset link:');
    console.log(`${SITE_URL}/reset-password/?token=${token}`);
    return;
  }
      ...
      html: `
        <div style="max-width:480px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
          ...
          <div style="padding:2rem;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;">
            ...
            <p style="font-size:0.75rem;color:#aaa;line-height:1.5;">이 링크는 1시간 동안 유효합니다.</p>
          </div>
        </div>
      `,
    });
